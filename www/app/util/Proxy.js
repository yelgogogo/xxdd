    ...
    var successCallback = function (resp, ops) {
        ...
        Ext.Array.each(Json_Order.Orders, function (order) {
            ...
            if (orderModel.data.OpCode > pn ){
               if(pn!='0'){
               sum += total
               var temp=Ext.create('app.model.Order', { 'Unit': '共计','SubTotal': total,'OpCode':pn});
               orderStore.add(temp);
...
//           if(pn==orderModel.data.OpCode){
               sum += total
               temp=Ext.create('app.model.Order', { 'Unit': '共计','SubTotal': total,'OpCode':pn});
               orderStore.add(temp);
               temp=Ext.create('app.model.Order', { 'GoodsName': '--------------------------------','GoodsTypeName': '--------', 'Unit': '-------','SubTotal': '-------', 'PresentUser': '----------------------','OpCode':pn});
               orderStore.add(temp);
//           };
      ...
      //      return Orders.get('OpCode') != app.CurRoom.RoomOpCode
      //  });
        //orderModel = Ext.create('app.model.Order', { 'GoodsName': '菜品', 'Price': '价格', 'Unit': '', 'GoodsCount': '数量', 'SubTotal': '小计', 'PresentUser': '落单人' });
